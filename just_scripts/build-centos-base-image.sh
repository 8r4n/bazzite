...
validate_install_root() {
    ...
    for repo_name in BaseOS AppStream; do
        [[ -d "${install_root}/${repo_name}/repodata" ]] || die "CentOS install root is missing ${repo_name}/repodata: ${install_root}"
    done
}

if [[ ${base_image_name} != "centos-stream-10" ]]; then
    die "This helper only supports CentOS Stream 10 builds."
fi
...
if [[ -n ${BAZZITE_CENTOS_INSTALL_ISO:-} && -n ${BAZZITE_CENTOS_INSTALL_ROOT:-} ]]; then
    die "Set either BAZZITE_CENTOS_INSTALL_ISO or BAZZITE_CENTOS_INSTALL_ROOT, not both."
fi

if [[ -z ${BAZZITE_CENTOS_INSTALL_ISO:-} && -z ${BAZZITE_CENTOS_INSTALL_ROOT:-} ]]; then
    die "Set BAZZITE_CENTOS_INSTALL_ISO or BAZZITE_CENTOS_INSTALL_ROOT before building the CentOS base image."
fi

staged_install_root="${project_root}/just_scripts/output/centos-install-root"
...
if [[ -n ${BAZZITE_CENTOS_INSTALL_ISO:-} ]]; then
    ...
    install_iso=$(readlink -f "${BAZZITE_CENTOS_INSTALL_ISO}")
    [[ -f ${install_iso} ]] || die "CentOS install ISO not found: ${BAZZITE_CENTOS_INSTALL_ISO}"
    ...
    install_root=$(readlink -f "${BAZZITE_CENTOS_INSTALL_ROOT}")
    [[ -d ${install_root} ]] || die "CentOS install root not found: ${BAZZITE_CENTOS_INSTALL_ROOT}"
...
workdir=$(mktemp -d "${TMPDIR:-/tmp}/bazzite-centos-base.XXXXXX")
...
cat > "${reposdir}/bazzite-centos-install-root.repo" <<EOF
...
enabled_repos=(bazzite-centos-baseos bazzite-centos-appstream)
...
repo_gpgcheck=0
EOF
    enabled_repos+=(bazzite-centos-crb)
fi
...
log "Installing CentOS base filesystem from local ISO-backed repos"
sudoif dnf5 -y \
    --installroot "${rootfs}" \
...
FROM scratch
ADD rootfs.tar /
LABEL org.bazzite.centos-install-source-kind="${install_source_kind}"
LABEL org.bazzite.centos-install-source-id="${install_source_id}"
LABEL org.bazzite.centos-install-source-sha256="${install_source_sha256}"
CMD ["/sbin/init"]
EOF

base_image_ref="${BAZZITE_CENTOS_BASE_IMAGE_OUTPUT:-localhost/bazzite-centos-stream-10-base:stream10}"
versioned_base_ref="localhost/bazzite-centos-stream-10-base:${build_version}-${git_branch_tag}"
