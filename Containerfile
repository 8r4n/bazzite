...
RUN --mount=type=cache,dst=/var/cache \
    ...
    if [[ "${BASE_IMAGE_NAME}" == "centos-stream-10" || "${BASE_IMAGE_NAME}" == "rhel-10" ]]; then \
        ...
        { curl --retry 3 -Lo /etc/flatpak/remotes.d/flathub.flatpakrepo https://dl.flathub.org/repo/flathub.flatpakrepo || true; } && \
        /ctx/image-info && \
        /ctx/build-initramfs && \
        /ctx/finalize \
    ; else \
        ...
        if grep -q "kinoite" <<< "${BASE_IMAGE_FAMILY}"; then \
...
ARG BASE_IMAGE_NAME="${BASE_IMAGE_NAME:-kinoite}"
ARG BASE_IMAGE_FAMILY="${BASE_IMAGE_FAMILY:-kinoite}"
ARG BASE_VARIANT_NAME="${BASE_VARIANT_NAME:-Kinoite}"
ARG BASE_VERSION="${BASE_VERSION:-43}"
ARG FEDORA_VERSION="${FEDORA_VERSION:-43}"
ARG VERSION_TAG="${VERSION_TAG}"
ARG VERSION_PRETTY="${VERSION_PRETTY}"

COPY system_files/deck/shared system_files/deck/${BASE_IMAGE_FAMILY} /
...
RUN --mount=type=cache,dst=/var/cache \
    ...
    if grep -q "silverblue" <<< "${BASE_IMAGE_FAMILY}"; then \
...
# Fetch NVIDIA driver
COPY system_files/nvidia/shared system_files/nvidia/${BASE_IMAGE_FAMILY} /
